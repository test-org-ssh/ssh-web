;(function ($) {
    ...
    var privateMethods = {
        initSliderBarCss : function(obj, opts){
            ...
            obj.find('.meeting-title').css(titleCss).find('i').css({
                'font-size': '15px'
            });
        },
        showAtLeft : function(obj, opts){
            if(opts.open){
                obj.css({left:'0px'});
                obj.find('.meeting-title').css('right','-25px').find('i').attr('class','fa fa-chevron-circle-left');
            }else{
                obj.css({left:-opts.width-22+'px'});
                obj.find('.meeting-title').css('right','-25px').find('i').attr('class','fa fa-chevron-circle-right');
            }

            obj.find('.meeting-title').click(function(){
                if(obj.data('open')){
        ...
        showAtRight : function(obj, opts){
            if(opts.open){
                obj.css({right:'0px'});
                obj.find('.meeting-title').css('right', opts.width+20+'px').find('i').attr('class','fa fa-chevron-circle-right');
            }else{
                obj.css({right:'25px'});
                obj.find('.meeting-title').css('right', opts.width+20+'px').find('i').attr('class','fa fa-chevron-circle-left');
            }
